...
def GetGPIO(raspi):
	GPIO = {}
	try:
		GPIODatei = open(raspi,"r")
		for zeile in GPIODDatei.readlines():
			zeile=zeile.replace("\n","")
			zeile=zeile.replace("\r","")
			try:
				PinNR,PinName = zeile.split(":")
			except KeyError:			
				print "Inkonsistenz in GPIODatei von ", raspi, ". PinNr. zu int konvertierbar?"
				time.sleep(4)
				GPIO = GetGPIO(raspi)
				pass
			GPIO[PinName]=int(PinNR)
	except ValueError:
		try:		
			print "Inkonsistenz in GPIODatei von ", raspi, ". PinNr. zu int konvertierbar?"
			time.sleep(4)
