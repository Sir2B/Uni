...
def SensornameZuTemperatur2Datei(): #Speichert Dictionary Serialisiert
	...
	TempSensornameZuTemperatur = pickle.load (open ("/home/pi/exchange/SensornameZuTemperatur","rb")) #Listeninhalt aktualisieren
	for Key in TempSensornameZuTemperatur.keys():
		try:
			SensornameZuTemperatur[Key]
		except KeyError:
			SensornameZuTemperatur[Key] = TempSensornameZuTemperatur[Key]	
...
def DateipfadZuSensorname2Datei(): #Speichert Dictionary Serialisiert
	...
	TempDateipfadZuSensorname = pickle.load (open ("/home/pi/exchange/DateipfadZuSensorname","rb")) #Listeninhalt aktualisieren
	for Key in TempDateipfadZuSensorname.keys():
		try:
			DateipfadZuSensorname[Key]
		except KeyError:
			DateipfadZuSensorname[Key] = TempDateipfadZuSensorname[Key]
...
def SensornameZuFile2Datei(): #Speichert Dictionary Serialisiert
	...
	TempSensornameZuFile = pickle.load (open ("/home/pi/exchange/SensornameZuFile","rb")) #Listeninhalt aktualisieren
	for Key in TempSensornameZuFile.keys():
		try:
			SensornameZuFile[Key]
		except KeyError:
			SensornameZuFile[Key] = TempSensornameZuFile[Key]

	Datei = open("/home/pi/exchange/SensornameZuFile","wb")
	Datei.write ( pickle.dumps(SensornameZuFile))
	Datei.close()	
	
def Aktualisierung(): #Aktualisiert alle Dictionaries
	global SensornameZuTemperatur
	global SensornameZuFile
	global DateipfadZuSensorname
	try:
		SensornameZuTemperatur = pickle.load (open ("/home/pi/exchange/SensornameZuTemperatur","rb")) 
	except IOError:
		SensornameZuTemperatur = {}
		pass
	except:
	...
	try:
		...
		pass
	try:	
		SensornameZuFile = pickle.load (open ("/home/pi/exchange/SensornameZuFile","rb")) 
	except IOError:
		SensornameZuFile = {}
		pass
