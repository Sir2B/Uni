...
class Puffer:
	def __init__(self,Volumen,EntlPumpe,LadePumpe,Aufstellort):	
		self.Volumen = Volumen
		self.Entladepumpe = GP
		self.Aufstellort = Aufstellort
		self.Beladepumpe = GPIO.... (LadePumpe,GPIO.out)
		self.Temperatur1 = []
		self.Ladezustand #Ladezustand in %
		
	def ErmittleLadezustand():
		SensorZuTemperatur = GetSensorZuTemperatur()   #Ladezustand ermitteln
		for Sensor in SensorZuTemperatur.keys():
			for i in range(6):
				if Sensor == ("SensorPuffer"+self.Aufstellort+str(i)):
					self.Temperatur.append(SensorZuTemperatur[Sensor])
